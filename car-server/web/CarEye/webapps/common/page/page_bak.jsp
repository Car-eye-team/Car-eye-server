...
<%if (myPage.getTotalPages() >0){ %>
		<div class="pageboxlist">
		 每页显示
			...
			<select id="pageSelected" onchange="jumpPage(1, <%=myPage.getTotalPages()%>)">
		    	<option value="10" <%if(myPage.getEveryPage() == 10){ %>selected="selected"<%} %>>10</option>
		        <option value="20" <%if(myPage.getEveryPage() == 20){ %>selected="selected"<%} %>>20</option>
		        <option value="30" <%if(myPage.getEveryPage() == 30){ %>selected="selected"<%} %>>30</option>
		        <option value="40" <%if(myPage.getEveryPage() == 40){ %>selected="selected"<%} %>>40</option>
		        <option value="50" <%if(myPage.getEveryPage() == 50){ %>selected="selected"<%} %>>50</option>
		    </select>
			...
			<%}else{%>
			    <%if(myPage.getCurrentPage()>4 && myPage.getCurrentPage() < (myPage.getTotalPages()-3)){ %>
					<% for(int a = (myPage.getCurrentPage()-4); a<(myPage.getCurrentPage()-1);a++){%>
						<a href='javascript:jumpPage(<%=a %>, <%=myPage.getTotalPages() %>)'><%=a %></a>	
					<%} %>
					<span id="currentPage"><%=myPage.getCurrentPage() %></span>					
					<% for(int b = (myPage.getCurrentPage()+1); b<(myPage.getCurrentPage()+3);b++){%>
				...
				<%}else if(myPage.getCurrentPage() >= (myPage.getTotalPages()-3)){ %>
					<% for(int d = (myPage.getTotalPages()-7); d< myPage.getTotalPages();d++){%>
            ...
            <span>
            	到第
            		<input type="text" id="pageNumber" value="<%=myPage.getCurrentPage() %>" /> 
            	页
            </span>
            <a href="javascript:void(0)" onclick="turnPage(<%=myPage.getTotalPages() %>)">确定</a>
		</div>
<%} %>
