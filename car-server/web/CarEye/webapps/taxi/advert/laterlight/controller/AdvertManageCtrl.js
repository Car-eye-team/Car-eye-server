Ext.define('AdvertManageApp.controller.AdvertManageCtrl', {
...
//    ],
	...
	buttonAccess : function() {
		var buttonArray = ['19010201','19010202','19010203','19010204','19010205','19010206','19010207'];
	...
	buttonAccess1 : function() {
		var buttonArray = ['19010208','19010209','19010210'];
		for(var i=0;i<buttonArray.length;i++){
			if(parent.menuidArray.indexOf(buttonArray[i]) == -1){
				Ext.getCmp(buttonArray[i]).hide();
			}
	...
	addVer : function(button){
		...
		var view = Ext.create('AdvertManageApp.view.AdvertVerAddView');
		view.show();
	...
	editVer : function(button){
		...
		var view = Ext.create('AdvertManageApp.view.AdvertVerEditView');
			view.show();
		var store = this.getAdvertVerStoreStore();
	...
	saveVer : function(button){
		...
		form.getForm().submit({
			url : window.BIZCTX_PATH + '/advertmanagejson/saveVer.action',
			method : 'post',
			waitMsg:'请稍候...',
				...
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
								url : window.BIZCTX_PATH
										+ '/advertmanagejson/deleteVer.action',
								method : 'POST',
								params : "ids=" + jsonData,
	...
	exportVer : function (button){
		...
		location.href=window.BIZCTX_PATH + '/advertmanagejson/exportVer.action?version='+version+
	...
	auditVer : function(button){
		var button = Ext.getCmp('19010205');
		var grid = button.up('grid');
		var records = grid.getSelectionModel().getSelection();
		if(records.length != 1){
		...
		var view = Ext.create('AdvertManageApp.view.AdvertVerAuditView');
			view.show();
		var store = this.getAdvertVerStoreStore();
		var data = store.getById(records[0].get('id'));
			view.down('form').loadRecord(data);
	...
	saveAuditVer : function(button){
		...
		form.getForm().submit({
			url : window.BIZCTX_PATH + '/advertmanagejson/saveAuditVer.action',
			method : 'post',
			waitMsg:'请稍候...',
			success : function(form, action) {
				var returnType = action.result.result.returnType;
	...
	activeVer : function (button){
			var button = Ext.getCmp('19010206');
			var grid = button.up('grid');
				...
				Ext.MessageBox.confirm('提示','你确认要上架选中的广告版本？',
								function(btn) {
									if (btn == "yes") {
										...
										Ext.Ajax.request( {
													url : window.BIZCTX_PATH + '/advertmanagejson/activeVer.action',
													method : 'POST',  
													params : "ids="+ jsonData,
	...
	inactiveVer : function(){
		var button = Ext.getCmp('19010207');
		var grid = button.up('grid');
		var data = grid.getSelectionModel().getSelection();
		var store = grid.getStore();
		if (data.length>0) {
			Ext.MessageBox.confirm('提示','你确认要下架选中的广告版本？',
							function(btn) {
								if (btn == "yes") {
									...
									Ext.Ajax.request( {
												url : window.BIZCTX_PATH + '/advertmanagejson/inactiveVer.action',
												method : 'POST',  
												params : "ids="+ jsonData,
												success : function(response) {
													store.reload();
	...
	addCon : function(button){
		...
		var view = Ext.create('AdvertManageApp.view.AdvertConAddView');
		view.show();
	...
	editCon : function(button){
		...
		var view = Ext.create('AdvertManageApp.view.AdvertConEditView');
			view.show();
		var store = this.getAdvertConStoreStore();
		var data = store.getById(records[0].get('id'));
			view.down('form').loadRecord(data);
	...
	saveCon : function(button){
		...
		form.getForm().submit({
			url : window.BIZCTX_PATH + '/advertmanagejson/saveCon.action',
			method : 'post',
			waitMsg:'请稍候...',
			success : function(form, action) {
				var returnType = action.result.result.returnType;
		...
		if (records.length > 0) {
			...
			Ext.Msg.confirm('提示', '你确认要删除选中的数据?', function(btn) {
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
								url : window.BIZCTX_PATH
										+ '/advertmanagejson/deleteCon.action',
								method : 'POST',
								params : "ids=" + jsonData,
								success : function(response, opts) {
									var text = response.responseText;
	...
	uploadFile : function(){ 
		   ...
		   'script': window.BIZCTX_PATH + '/advertmanagejson/advertUpload.action',   //指定服务端处理类的入口
