...
Ext.define('OperaMileStatiApp.controller.OperaMileStatiCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['CarListStore','OperaMileStatiListStore','CarPageListStore'],//声明该控制层要用到的store
    models: [],//声明该控制层要用到的model
    views: ['OperaMileStatiListView','OperaMileStatiSearchView'],//声明该控制层要用到的view
    refs: [//相当于一个映射,这样就可以在控制层方便的通过geter取得相应的对象了
        {
            ref: 'operaMileStatiListView',
            selector: 'operaMileStatiListView'
        }
    ],
    init: function() {
    	
		this.control({
			'operaMileStatiSearchView #blocid' : {
				select : this.loadDeptCar
			},
			'operaMileStatiSearchView button[action=search]' : {
				click : this.search
			},
			'operaMileStatiListView button[action=export]' : {
	...
	exportExcel : function (button){
    	...
    	location.href= window.BIZCTX_PATH + '/reportformsjson/exportOms.action?carnumber='+carnumber
	...
	search : function(){		
        ...
        var store = this.getOperaMileStatiListStoreStore();
		store.clearFilter(true);
		store.on('beforeload', function (store, options) {
	            var new_params = { 
