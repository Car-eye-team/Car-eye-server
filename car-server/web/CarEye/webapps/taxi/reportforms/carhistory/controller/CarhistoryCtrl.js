Ext.define('CarhistoryApp.controller.CarhistoryCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['CarhistoryStore','CarListStore','CarStatusStore','GpsFlagStore','CarPageListStore'],//声明该控制层要用到的store
    views: ['CarhistoryListView','SearchMapView','CarhistorySearchView','MapView'],//声明该控制层要用到的view
    refs: [//相当于一个映射,这样就可以在控制层方便的通过geter取得相应的对象了
        {
            ref: 'carhistoryListView',
            selector: 'carhistoryListView'
        }
    ],
    init: function() {
    	
		this.control({
			'carhistorySearchView #carnumber' : {
				change : this.loadCar
			},
			'mapView': {
            ...
            'carhistorySearchView #al_deptid' : {
...
//		maptype = 1;
	...
	initGaoDeMap : function(div) {
		...
		eventListener = AMap.event.addListener(mapObj,'complete',function(){
			...
			info.push("<font><b>创建时间：</b></font><font>"+(createtime ==undefined ? "" :createtime)+"</font>"); 
...
//						  height: 60,     // 信息窗口高度
						...
						var infoWindow = new BMap.InfoWindow("地址："+address+"</br>创建时间："+createtime, opts);  // 创建信息窗口对象
	...
	loadCar : function(){
		...
		var carnumber = Ext.getCmp('carhistory_carnumber').getRawValue();
    	if(admin.deptid == 1){
	...
	exportInfo : function (button){
        	...
        	var carnumber = encodeURI(Ext.getCmp('carhistory_carnumber').getValue());
        	var terminal = encodeURI(Ext.getCmp('carhistory_terminal').getValue());
        	var gpsflag = Ext.getCmp('carhistory_gpsflag').getValue();
        	var carstatus = Ext.getCmp('carhistory_carstatus').getValue();
			var stime = encodeURI(Ext.util.Format.date(Ext.getCmp('carhistory_stime').getValue(), 'Y-m-d H:i:s'));
			var etime = encodeURI(Ext.util.Format.date(Ext.getCmp('carhistory_etime').getValue(), 'Y-m-d H:i:s'));
        	location.href= window.BIZCTX_PATH + '/reportformsjson/exportCarhistory.action?carnumber='+carnumber
        	+'&terminal='+terminal+'&stime='+stime+'&etime='+etime+'&blocid='+blocid
        	+'&gpsflag='+gpsflag+'&carstatus='+carstatus;
	},
	search :function(){
		var store = this.getCarhistoryStoreStore();
		store.clearFilter(true);
		store.on('beforeload', function (store, options) {
	            var new_params = { 
	            	gpsflag : Ext.getCmp('carhistory_gpsflag').getValue(),
	            	carstatus : Ext.getCmp('carhistory_carstatus').getValue(),
	            	blocid: Ext.getCmp('al_deptid').getValue(),
	            	carnumber: encodeURI(Ext.getCmp('carhistory_carnumber').getValue()),
	            	terminal: encodeURI(Ext.getCmp('carhistory_terminal').getValue()),
        			stime : encodeURI(Ext.util.Format.date(Ext.getCmp('carhistory_stime').getValue(), 'Y-m-d H:i:s')),
        			etime : encodeURI(Ext.util.Format.date(Ext.getCmp('carhistory_etime').getValue(), 'Y-m-d H:i:s'))
	            };
	            Ext.apply(store.proxy.extraParams, new_params);
	        });
	        store.loadPage(1); 
		...
		if (data.length>0) {
			...
			function(btn) {
				if (btn == "yes") {
					...
					Ext.Ajax.request( {
								url : window.BIZCTX_PATH + '/reportformsjson/deleteCarhistory.action',
								method : 'POST',  
