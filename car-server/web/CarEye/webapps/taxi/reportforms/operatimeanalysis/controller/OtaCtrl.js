...
Ext.define('OtaApp.controller.OtaCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['CarListStore','OtaListStore','CarPageListStore'],//声明该控制层要用到的store
    models: [],//声明该控制层要用到的model
    views: ['OtaListView','OtaSearchView','OtaDataView','DbaData'],//声明该控制层要用到的view
    refs: [//相当于一个映射,这样就可以在控制层方便的通过geter取得相应的对象了
        {
            ref: 'otaListView',
            selector: 'otaListView'
        }
    ],
    init: function() {
    	
		this.control({
			'otaSearchView #al_deptid' : {
	...
	exportOta : function (button){
    	...
    	location.href= window.BIZCTX_PATH + '/reportformsjson/exportOta.action?carnumber='+carnumber
    	+'&stime='+stime+'&etime='+etime+'&blocid='+blocid;
     
	},
	
	searchData:function(){
		var store = this.getOtaListStoreStore();
		store.clearFilter(true);
		store.on('beforeload', function (store, options) {
	            var new_params = { 
	            		blocid : Ext.getCmp('al_deptid').getValue(),
	            		carnumber : encodeURI(Ext.getCmp('ota_carnumber').getValue()),
	...
	search : function(){
		...
		var con = this;
		Ext.Ajax.request({
		      url: window.BIZCTX_PATH + '/reportformsjson/queryOta.action', //请求的服务器地址
		      method : 'POST',
		      params : {
					blocid:blocid,
