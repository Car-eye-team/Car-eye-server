Ext.define('CarrealtimeApp.controller.CarrealtimeCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['CarrealtimeStore','CarListStore','CarStatusStore','GpsFlagStore','CarPageListStore'],//声明该控制层要用到的store
    views: ['CarrealtimeListView','SearchMapView','CRSearchView','MapView'],//声明该控制层要用到的view
    refs: [//相当于一个映射,这样就可以在控制层方便的通过geter取得相应的对象了
        {
            ref: 'carrealtimeListView',
            selector: 'carrealtimeListView'
        }
    ],
    init: function() {
    	
		this.control({
			'cRSearchView #carrealtime_carnumber' : {
				change : this.loadCar
			},
			'mapView': {
	...
	loadDeptCar : function(){
		...
		Ext.getCmp('carrealtime_carnumber').setValue("");
...
//						  height: 60,     // 信息窗口高度
	...
	exportInfo : function (button){
			var carstatus = Ext.getCmp('carrealtime_carstatus').getValue();
			var gpsflag = Ext.getCmp('carrealtime_gpsflag').getValue();
		    var blocid = Ext.getCmp('al_deptid').getValue();
        	var carnumber = Ext.getCmp('carrealtime_carnumber').getValue();
        	var terminal = Ext.getCmp('carrealtime_terminal').getValue();
			var stime = encodeURI(Ext.util.Format.date(Ext.getCmp('carrealtime_stime').getValue(), 'Y-m-d H:i:s'));
			var etime = encodeURI(Ext.util.Format.date(Ext.getCmp('carrealtime_etime').getValue(), 'Y-m-d H:i:s'));
        	location.href= window.BIZCTX_PATH + '/reportformsjson/exportCarrealtime.action?carnumber='+carnumber
        	+'&terminal='+terminal+'&stime='+stime+'&etime='+etime+'&blocid='+blocid+'&carstatus='+carstatus+'&gpsflag='+gpsflag;
	},
	search :function(){
		var store = this.getCarrealtimeStoreStore();
		store.clearFilter(true);
		store.on('beforeload', function (store, options) {
	            var new_params = { 
	            	carstatus : Ext.getCmp('carrealtime_carstatus').getValue(),
	            	gpsflag : Ext.getCmp('carrealtime_gpsflag').getValue(),
	            	blocid : Ext.getCmp('al_deptid').getValue(),
	            	carnumber : encodeURI(Ext.getCmp('carrealtime_carnumber').getValue()),
	            	terminal : encodeURI(Ext.getCmp('carrealtime_terminal').getValue()),
        			stime : encodeURI(Ext.util.Format.date(Ext.getCmp('carrealtime_stime').getValue(), 'Y-m-d H:i:s')),
        			etime : encodeURI(Ext.util.Format.date(Ext.getCmp('carrealtime_etime').getValue(), 'Y-m-d H:i:s'))
	            };
	            Ext.apply(store.proxy.extraParams, new_params);
	        });
	        store.loadPage(1); 
