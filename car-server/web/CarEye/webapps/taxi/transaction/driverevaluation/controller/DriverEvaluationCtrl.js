Ext.define('DriverEvaluationApp.controller.DriverEvaluationCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['DriverEvaluationListStore','SlevelStore'],//声明该控制层要用到的store
    models: ['DriverEvaluationModel'],//声明该控制层要用到的model
    views: ['DriverEvaluationSearchView','DriverEvaluationListView'],//声明该控制层要用到的view
    ...
    init: function() {
		...
		this.control({
			'driverEvaluationSearchView button[action=search]' : {
				click : this.search
			}
...
//		}
	...
	search : function(button) {
		...
		var store = this.getDriverEvaluationListStoreStore();
	...
	exportExcel : function (button){
		...
		location.href=window.BIZCTX_PATH + '/driverevaluationjson/exportExcel.action?dname='+dname+"&cname="+cname+
