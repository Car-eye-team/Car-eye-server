Ext.define('SearchRecordApp.controller.SearchRecordCtrl', {
    extend: 'Ext.app.Controller',
    stores: ['SearchRecordListStore','CarListStore','EventCodeStore','SearchRecordTypeStore','CarPageListStore'],//声明该控制层要用到的store
    views: ['SearchRecordListView','SearchRecordSearchView'],//声明该控制层要用到的view
	...
	search :function(){
		...
		var store = this.getSearchRecordListStoreStore();
		store.clearFilter(true);
		store.on('beforeload', function (store, options) {
	            var new_params = { 
	            	carnumber: encodeURI(Ext.getCmp('m_carnumber').getValue()),
	...
	excelExport:function(button){
		...
		var stime = encodeURI(Ext.util.Format.date(Ext.getCmp('m_stime').getValue(), 'Y-m-d H:i:s'));
		var etime = encodeURI(Ext.util.Format.date(Ext.getCmp('m_etime').getValue(), 'Y-m-d H:i:s'));
		
		location.href=window.BIZCTX_PATH + '/multimedia/multimediajson/exportMultimediaSearchRecord.action?carnumber='+carnumber+"&mediatype="+mediatype+
