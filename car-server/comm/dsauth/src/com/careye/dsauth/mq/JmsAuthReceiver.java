...
import com.careye.dsauth.business.ReceiveBusiness;
import com.careye.dsauth.constant.ConfigProperties;
import com.careye.dsauth.mq.utils.ClientTransportListener;
...
public class JmsAuthReceiver implements MessageListener {
	...
	public void init(){
		...
		try{
			...
			destination = session.createQueue(ConfigProperties.DS_AUTH_UP_QUEUE_NAME);  
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof MapMessage) {
				...
				if(msg != null){
					ReceiveBusiness.mqMessageReceived(msg);
				}else{
					logger.info("接收鉴权服务器上行队列消息为空!");
					restart();
				}
			} else {
				logger.info("接收鉴权服务器上行队列非约定格式消息,系统不处理: " + message);
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("接收鉴权服务器上行消息异常,系统重启消息接收"+e);
			restart();
		}
	}
