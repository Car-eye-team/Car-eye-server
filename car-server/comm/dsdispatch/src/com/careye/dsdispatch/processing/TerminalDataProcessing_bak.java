 ...
 *     
 */
public class TerminalDataProcessing_bak {

	private final static Logger logger = Logger.getLogger(TerminalDataProcessing_bak.class);
			...
			if(msgid == 512){
				...
				if(jsonObject != null){
					...
					if(lng == 0 || lat == 0){
						...
						//获取上一次位置的mapkey值
						TerminalInfo tInfo = (TerminalInfo) RedisManager.getInstance().getObject(mapkeybBuffer.toString());
						if(tInfo == null){
						...
						RedisManager.getInstance().setObject(mapkeybBuffer.toString(),tInfo);
					}
				}
			}
