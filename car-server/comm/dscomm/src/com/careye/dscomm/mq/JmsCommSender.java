 ...
 * 类描述：MQ中写入消息   
 * 创建人：zr    
 * 创建时间：2015-6-30 下午12:55:42    
 * 修改人：zr    
 * 修改时间：2015-6-30 下午12:55:42    
 * 修改备注：    
 * @version 1.0  
 *     
 */
public class JmsCommSender implements ExceptionListener{
	...
	protected static JmsCommSender jmsCommSender = null;

	/**
	 * 获取唯一实例.
	 * @return
	 */
	public static JmsCommSender getInstance(){
		if (jmsCommSender == null) {
			jmsCommSender = new JmsCommSender();
		}
		return jmsCommSender;
	}

	public JmsCommSender() {
		init();
	}
	...
	private void sendMsg(Map<String, Object> map) throws Exception {
		...
		try {
			...
			//PTP消息方式     
			Destination destination = session.createQueue(ConfigProperties.DS_COMM_UP_QUEUE_NAME);
