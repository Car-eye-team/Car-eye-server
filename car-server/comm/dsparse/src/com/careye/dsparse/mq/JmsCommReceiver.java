 ...
 * @version 1.0  
 */
public class JmsCommReceiver implements MessageListener {

	private final static Logger logger = Logger.getLogger(JmsCommReceiver.class);
	...
	protected static JmsCommReceiver jmsCommReceiver = new JmsCommReceiver();

	public JmsCommReceiver(){
		//start();
	}
	 ...
	 * @return
	 */
	public static JmsCommReceiver getInstance(){
		if (jmsCommReceiver == null) {
			jmsCommReceiver = new JmsCommReceiver();
		}
		return jmsCommReceiver;
	}
	...
	public void init(){
		...
		try{
			...
			destination = session.createQueue(ConfigProperties.DS_COMM_UP_QUEUE_NAME);  
			...
			if (message instanceof MapMessage) {
				...
				if(msg != null){
					if(ThreadPoolManager.executor!=null){
						...
						logger.info("[接收接入服务器上行队列消息] [threadpool] ===="+ThreadPoolManager.executor.getPoolSize()+"==="+ThreadPoolManager.executor.getLargestPoolSize()+"==="+ThreadPoolManager.queue.size());
						ThreadPoolManager.executor.execute(new ThreadPoolTask(msg));
					}
				}else {
					logger.info("接收接入服务器上行队列消息为空!");
				}
				
			} else {
				logger.info("接收接入服务器上行队列消息非约定格式消息,系统不处理"+message);
			}
		} catch (Exception e) {
			e.printStackTrace();
			ExceptionUtil.getInfo(e);
			logger.info("接收接入服务器上行消息异常,系统重启消息接收"+e);
			restart();
		}
	}
