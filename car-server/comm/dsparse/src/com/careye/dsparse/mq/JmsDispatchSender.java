 ...
 *     
 */
public class JmsDispatchSender implements ExceptionListener{
	...
	protected static JmsDispatchSender jmsDispatchSender = null;

	/**
	 * 获取唯一实例.
	 * @return
	 */
	public static JmsDispatchSender getInstance(){
		if (jmsDispatchSender == null) {
			jmsDispatchSender = new JmsDispatchSender();
		}
		return jmsDispatchSender;
	}

	public JmsDispatchSender() {
		init();
	}
	...
	private void sendMsg(Map<String, Object> map) throws Exception {
		...
		try {
			...
			//PTP消息方式     
			Destination destination = session.createQueue(ConfigProperties.DS_DISPATCH_DOWN_QUEUE_NAME);
