 ...
 *     
 * 项目名称：dsparse    
 * 类名称：JmsDispatchSender    
 * 类描述：调度服务器下行消息队列 主要跑心跳消息  
 * 创建人：zr    
 * 创建时间：2015-5-21 下午02:47:51    
 * 修改人：zr    
 * 修改时间：2015-5-21 下午02:47:51    
 * 修改备注：    
 * @version 1.0  
 *     
 */
public class JmsDispatchSenderHeartbeat implements ExceptionListener{
	...
	protected static JmsDispatchSenderHeartbeat jmsDispatchSenderHeartbeat = null;

	/**
	 * 获取唯一实例.
	 * @return
	 */
	public static JmsDispatchSenderHeartbeat getInstance(){
		if (jmsDispatchSenderHeartbeat == null) {
			jmsDispatchSenderHeartbeat = new JmsDispatchSenderHeartbeat();
		}
		return jmsDispatchSenderHeartbeat;
	}

	public JmsDispatchSenderHeartbeat() {
		init();
	}
	...
	private void sendMsg(Map<String, Object> map) throws Exception {
		...
		try {
			...
			//PTP消息方式     
			Destination destination = session.createQueue(ConfigProperties.DS_DISPATCH_HEARTBEAT_DOWN_QUEUE_NAME);
