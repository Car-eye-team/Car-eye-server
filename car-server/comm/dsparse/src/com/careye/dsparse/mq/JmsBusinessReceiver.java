...
import com.careye.dsparse.business.ReceiveBusinessMqBusiness;
 ...
 *     
 * 项目名称：dsparse    
 * 类名称：JmsBusinessReceiver    
 * 类描述：业务分发服务器下行MQ消息读取  
 * 创建人：zr    
 * 创建时间：2015-5-12 下午07:05:06    
 * 修改人：zr    
 * 修改时间：2015-5-12 下午07:05:06    
 * 修改备注：    
 * @version 1.0  
 *
 */
public class JmsBusinessReceiver implements MessageListener {
	...
	public void init(){
		...
		try{
			...
			destination = session.createQueue(ConfigProperties.DS_BUSINESS_DOWN_QUEUE_NAME);  
	...
	public void onMessage(Message message) {
		try {
			if (message instanceof MapMessage) {
				...
				if(msg != null){
					ReceiveBusinessMqBusiness.mqMessageReceived(msg);
				}else {
					logger.info("接收业务分发服务器下行队列消息为空!");
				}
			} else {
				logger.info("接收业务分发服务器下行队列非约定格式消息,系统不处理: " + message);
			}
		} catch (Exception e) {
			e.printStackTrace();
			ExceptionUtil.getInfo(e);
			logger.info("接收业务分发服务器下行消息异常,系统重启消息接收"+e);
			restart();
		}
	}
	...
	public void restart(){
		...
		logger.info("===接入业务分发服务器下行MQ消息连接重启===");
		try {
			close();
			Thread.sleep(1000);
			start();
